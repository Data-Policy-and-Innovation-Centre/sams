...
def get_existing_modules(table_name: str = "students", db_path=f"{RAW_DATA_DIR}/sams.db") -> list:
    ...
    if table_name == "students":
        ...
        modules_with_duplicates = [(module, year, num_obs) for module, year, num_obs in existing_modules
                       if num_obs > counts[(counts["module"] == module) & (counts["academic_year"] == year)]["count"].values[0]]
        if modules_with_duplicates:
            logger.warning(f"Modules with excess records than expected found: {modules_with_duplicates}")
