...
gender_ratio$GenderRatio = gender_ratio$Female/gender_ratio$Male*100

ggplot(gender_ratio, aes(x = Year, y = GenderRatio, color = ReportedBranchORTrade, group = ReportedBranchORTrade)) +
  geom_line(size = 1) +
  labs(
    title = "Gender Ratio (Male/Female) Over Time by Trade in %",
    x = "Year",
    y = "Gender Ratio (%)",
    color = "Trade"
  ) +
  theme_minimal() +
  scale_color_manual(values = rainbow(length(core_trades))) 
