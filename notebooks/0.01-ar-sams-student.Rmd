...
ggplot(df_ITI_summary, aes(x = AnnualIncome, y = percentage, fill = Gender)) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", percentage)), 
            ...
            vjust = -0.5, size = 2) +
  labs(
    title = "Percentage Distribution of Income by Gender",
...
ggplot(df_ITI_summary, aes(x = ReligionName, y = percentage, fill = Gender)) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", percentage)), 
            ...
            vjust = -0.5, size = 2) +
  labs(
    title = "Percentage Distribution of Religion by Gender",
    x = "Religion",
    y = "Percentage",
    fill = "Gender"
  ) +
...
ggplot(df_ITI_summary, aes(x = SocialCategory, y = percentage, fill = Gender)) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", percentage)), 
            ...
            vjust = -0.5, size = 3) +
  labs(
    title = "Social Category Distribution by Gender",
    x = "Social Category",
    y = "Percentage",
    fill = "Gender"
  ) +
...
ggplot(df_ITI_summary, aes(x = CoursePeriod, y = percentage, fill = Gender)) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", percentage)), 
            ...
            vjust = -0.5, size = 3) +
  labs(
    title = "Course Period Distribution by Gender",
    x = "Course Period",
    y = "Percentage",
    fill = "Gender"
  ) +
...
ggplot(df_count_2023, aes(x = female_sub_category_2023, y=count )) +
  ...
  labs(title = "Distribution of Institutes by Female Percentage Category for 2023", x = "Female Percentage", y = "Number of ITIs", fill="Female Percentage Category") +
  ylim(0,35)
...
ggplot(gender_ratio, aes(x = Year, y = PercentFemale, color = ReportedBranchORTrade, group = ReportedBranchORTrade)) +
  geom_line(aes(linetype = ReportedBranchORTrade), size = 1) +
  ...
  # Customize the theme for a more sophisticated look
  theme_bw() +
  theme(
...
gender_ratio <- df_ITI %>%
  ...
  summarise(Count = n()) %>%
  spread(key = Gender, value = Count, fill = 0) %>%
  mutate(Total = Male + Female, PercentFemale = (Female / Total) * 100) # Calculate ratio as Male/Female
view(gender_ratio)
