...
```{r}
# Create file path
file_path = "C:\\Users\\Admin\\Documents\\GitHub\\sams\\output\\ITI_descriptives.xlsx"
...
ggplot() +
  ...
  geom_line(data = aggregated_data, aes(x = Year, y = Aggregate_Female_Percent), linetype = "dotted", color = "black", size = 1) +
  labs(title = "Female Share of Total Students Over Time",
       x = "Year",
       y = "Female Share") +
  scale_x_continuous(breaks = seq(min(ts_top_5_female_ITI_gov$Year), max(ts_top_5_female_ITI_gov$Year), by = 1)) + # Breaks every year
  ylim(0,50)
...
## SC/ST Share of Female in ITIs
# Filter for SC and ST students
sc_st_ITI <- df_ITI %>%
  filter(SocialCategory %in% c("Schedule Caste (SC)", "Schedule Tribe (ST)"))

# Calculate the female share over time, separately for Pvt. and Govt.
female_share_sc_st_ITI <- sc_st_ITI %>%
  group_by(Year, TypeofInstitute) %>%
  summarise(
    total_students = n(),
    female_students = sum(Gender == "Female"),
...
ggplot() +
  # Scatter points for mean female share by year (Govt. vs Pvt.)
  geom_point(data = female_share_sc_st_ITI, aes(x = Year, y = female_share, color = TypeofInstitute), 
             size = 2, shape = 17) +  # Change shape to distinguish mean points
