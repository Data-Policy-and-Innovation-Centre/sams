...
writeData(wb_ITI_proof, "Income Group Pvt ITIs", ITI_income_grp_gov)
...
ITI_income_grp_gender_pub = ITI_income_grp_gender_pub %>% 
  group_by(Gender) %>%
  bind_rows(
    ITI_income_grp_gender_pub %>%
      filter(AnnualIncome %in% c("2,50,000 - 6,00,000", "More than 8,00,000")) %>%
      summarise(across(count, sum, na.rm = FALSE)) %>%
      mutate(AnnualIncome = "2,50,000 and Above")
  ) %>%
  filter(!(AnnualIncome %in% c("2,50,000 - 6,00,000", "More than 8,00,000","NA","OTHER"))) %>%
  ungroup()
...
view(ITI_income_grp_gender_pub)
addWorksheet(wb_ITI_proof, "Income Group Gender-Wise Gov ITIs")
writeData(wb_ITI_proof, "Income Group Gender-Wise Gov ITIs", ITI_income_grp_gov)


# Gender-wise within Pvt
ITI_pvt_gender_soc_grp
