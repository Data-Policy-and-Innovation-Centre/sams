...
trades_by_quals_pct %>% head(10) %>% kable( caption = "Trades by highest qualifications in 2023 (%)") %>%
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
trades_by_gender_pct %>% right_join(top_10_trades_by_quals_grad,by="Trade") %>% select(Trade,Male,Female, Total.x) %>% rename(Total = Total.x) %>% kable( caption = "Trades by highest qualifications in 2023 (%)") %>%
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
qual_by_gender_pct %>%  kable( caption = "Qualifications by gender in 2023 (%)") %>%
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
```{r Number of institutes, echo=FALSE, include=TRUE}
...
```{r Distance travelled over time by gender, echo=FALSE, include=TRUE}
