...
# Block level enrollment (Work in Progress)
```{r}
...
## Why do students choose vocational training - ITI? 

# Is it due to poor marks in 10th?
```{r}
## Distribution of marks in 10th for ITI students
...
# Density plot ITI for only year 2023
ggplot(student_iti_admis %>% filter(year == 2023, social_category == "General",Qualifying_ExamName != "8th Pass"), aes(x = MarksPercentage, fill = typeof_institute)) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("blue", "red")) +
  geom_vline(xintercept = 33, linetype = "dashed", color = "red", size = 1) +
  annotate("text", x = 33, y = 0, label = "33", color = "red", vjust = 1.5, size = 2) +
  labs(title = "Density of Marks Percentage for Year 2023 (ITI)",
       x = "Marks Percentage",
       y = "Density") +
  coord_cartesian(xlim = c(0, 100)) + 
  theme_minimal()

ggplot(student_iti_admis %>% filter(year == 2023, social_category == "OBC/SEBC",Qualifying_ExamName != "8th Pass"), aes(x = MarksPercentage, fill = typeof_institute)) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("blue", "red")) +
  geom_vline(xintercept = 33, linetype = "dashed", color = "red", size = 1) +
  annotate("text", x = 33, y = 0, label = "33", color = "red", vjust = 1.5, size = 2) +
...
ggplot(student_iti_admis %>% filter(year == 2023, social_category == "Schedule Caste (SC)",Qualifying_ExamName != "8th Pass"), aes(x = MarksPercentage, fill = typeof_institute)) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("blue", "red")) +
  geom_vline(xintercept = 33, linetype = "dashed", color = "red", size = 1) +
  annotate("text", x = 33, y = 0, label = "33", color = "red", vjust = 1.5, size = 2) +
...
ggplot(student_iti_admis %>% filter(year == 2023, social_category == "Schedule Tribe (ST)",Qualifying_ExamName != "8th Pass"), aes(x = MarksPercentage, fill = typeof_institute)) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("blue", "red")) +
  geom_vline(xintercept = 33, linetype = "dashed", color = "red", size = 1) +
  annotate("text", x = 33, y = 0, label = "33", color = "red", vjust = 1.5, size = 2) +
...
# Density plot ITI filter by CBSE only for year 2023
ggplot(student_iti_admis %>% filter(year == 2023, HighestQualExamBoard == "Central Board of Secondary Education, 02-Community Centre, Shaiksha Kendra, Preet Vihar, Delhi-110301"), aes(x = MarksPercentage)) +
...
## Distribution of marks for Diploma students



student_iti_admis_2023 <- student_iti_admis %>% filter(year == 2023)



min_marks_df <- student_iti_admis_2023 %>%
  group_by(Qualifying_ExamName) %>%
  summarize(MaxMarksPercentage = max(MarksPercentage, na.rm = TRUE)) %>%
  ungroup()
