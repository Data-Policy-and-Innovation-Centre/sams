...
iti_total_students = student_iti_admis %>%
  filter(year != 2017) %>%
  group_by(year, typeof_institute) %>%
  summarise(total_students = n(), .groups = "drop") %>%
  # Step 2: Spread the data into separate columns for Pvt and Gov
  pivot_wider(names_from = typeof_institute, values_from = total_students, values_fill = 0) %>%
  # Step 3: Calculate total students, share of Pvt and Gov
  mutate(total_students = `Govt.` + `Pvt.`,
         share_of_pvt = (`Pvt.` / total_students) * 100,
         share_of_gov = (`Govt.` / total_students) * 100) %>%
  # Step 4: Select relevant columns
  select(year, total_students, share_of_pvt, share_of_gov) %>%
  arrange(year)

print(iti_total_students)
