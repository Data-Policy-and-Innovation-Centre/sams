...
Vocational training in Odisha encompasses a variety of programs aimed at equipping individuals with specialized skills for employment in a wide variety of industrial settings. The key components of vocational education in Odisha include Industrial Training Institutes (ITIs), Polytechnic (Diploma) programs, and Post-Diploma programs. Each of these educational pathways serves different levels of skill development, from basic trades to more specialized technical expertise.
...
While both the central and state governments have implemented a variety of initiatives to promote vocational training as a key tool for improving employment opportunities for youth, there remains a significant gap in its popularity, especially among students immediately graduating from the 10th grade in Odisha. This trend is a cause for concern, given that vocational training has the potential to equip these young individuals with the practical skills necessary for gainful formal employment in a rapidly evolving economy. 
...
*Geography tables to be replaced with maps today*


```{r ITI home states 2023, echo=FALSE}

html_table <- iti_home_states_2023 %>% kable(caption = "Home states of enrolled ITI trainees in 2023") %>% 
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
```{r Diploma home states 2023, echo=FALSE}

html_table <- iti_home_states_2023 %>% kable(caption = "Home states of enrolled Polytechnic students in 2023") %>% 
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
```{r ITI home districts 2023, echo=FALSE}

html_table <- iti_home_districts_2023 %>% kable(caption = "Home state of enrolled ITI trainees in 2023") %>% 
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
```{r Diploma home districts 2023, echo=FALSE}

html_table <- iti_home_districts_2023 %>% kable(caption = "Home state of enrolled Polytechnic students in 2023") %>% 
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
*We should move the location exhibit on source / destination here*
...
*Add more basic facts on enrollment by gender? Reference to Sudakhya scheme*
...
```
Setting suboptimal cutoffs in vocational training institutions can lead to significant inefficiencies, with some trades experiencing oversubscription while others face large vacancies. In popular trades like electrician and fitter, the demand for seats often exceeds availability, resulting in higher competition and oversubscription, particularly when cutoffs are set too low or fail to account for gender or category imbalances. This can overwhelm institutional resources and extend waiting times for students. Conversely, less popular trades may suffer from low demand and large vacancies when cutoffs are set too high across the board, leading to underutilization of resources and missed opportunities for both students and institutions. Ensuring an accurate, data-driven approach to setting cutoffs is essential for maintaining balance, optimizing seat allocation, and aligning training programs with market needs.
